...
满足进入条件后，Phase 2 仍然按薄切片推进，不允许绕过前置资产直接堆功能。

每个产品能力进入实现前必须满足：

- 对应需求能追溯到 PRD、需求修订或用户确认记录。
- UI 或交互变化能追溯到设计资产、工程规格或明确的实现约束。
- 后端写入必须先形成计划、确认策略和幂等规则。
- 前端必须通过 `@ai-code/sdk` 和契约访问后端，不直接散落业务端点。
- 阶段结束必须更新 `current-project-state.md`，并运行 `pnpm validate:context-sync`。
- 如果没有实际写入 Obsidian 或飞书，最终说明必须明确“仓库已更新，外部知识库未同步”。
