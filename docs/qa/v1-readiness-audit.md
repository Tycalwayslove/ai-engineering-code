...
| iOS 系统能力人工证据记录 | `pnpm validate:ios-manual-evidence-record`；`pnpm prepare:ios-manual-review-pack -- --record <review-path>`；`pnpm prepare:ios-manual-evidence-record -- --record <review-path> --output <filled-path>`；最终验收使用 `node scripts/validate-ios-manual-evidence-record.mjs --record <path> --require-complete --report <report-path>` | 校验人工证据记录结构；从 review 记录生成 Markdown / HTML 复核包、`manual-evidence-reviewed-items.json` 和 filled 草稿；completion 模式要求每项 `passed` 且证据字段齐全；`--mark-passed` 还必须传入与 `record.items[].id` 完全一致的 `--reviewed-item <item-id>` 列表或 `--reviewed-items-file <json-or-lines>`，Review Pack 生成的确认列表会携带 `recordHeadSha` 并在签署时与当前记录 `headSha` 校验；脚本会把 `reviewedItemIds` 写入 `operatorSignoff`；`--report` 生成补证缺口报告 | 已有命令，当前只能证明模板结构、review pack、逐项签署闸门、确认列表 HEAD 绑定和 filled 草稿入口可校验；review pack 会展示 required evidence、候选证据和可复制的 `--reviewed-items-file` 签署命令，HTML 版额外提供可点击证据路径，仍是操作者复核工作台，不代表人工验收通过；真实 completion 仍需人工复核并签署补证文件 |
