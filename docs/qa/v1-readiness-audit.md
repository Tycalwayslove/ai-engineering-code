...
| v1 completion audit 汇总 | `pnpm collect:v1-completion-audit`；正式收尾使用 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` | 生成单一 `v1-completion-audit.json` / `.md`，汇总自动化命令状态、人工证据记录 completion 校验、`externalKnowledgeSync` 外部知识库同步状态和 goal 是否可 complete；提供 `--manual-record` 时同目录归档 `manual-evidence-gaps.md` | 已有命令；默认不跑重型命令，只生成缺口审计；正式 completion audit 必须显式运行自动化命令并提供补证后的人工记录；外部知识库真实同步后可用 `--external-knowledge-status synced` 或 `--external-knowledge-synced` 记录证据 |
...
- 尚未在单一 completion audit 记录中留存全套自动化命令输出；`pnpm collect:v1-completion-audit` 已提供固定产物入口，且提供 `--manual-record` 时会把 `manual-evidence-gaps.md` 一并归档到审计目录，但正式收尾仍需使用 `--run-automated-commands --manual-record <path>` 重新生成完整记录。
...
5. `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` 生成的 `v1-completion-audit.json` / `.md` 结论为 `passed`，并归档自动化命令、人工证据记录路径、同目录 `manual-evidence-gaps.md` 和 `externalKnowledgeSync` 状态；若实际执行了飞书 / Obsidian 同步，需要通过 `--external-knowledge-status synced|partial`、`--feishu-sync-evidence <path>` 或 `--obsidian-sync-evidence <path>` 记录证据。
