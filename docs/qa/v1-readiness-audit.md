...
| v1 completion audit 汇总 | `pnpm collect:v1-completion-audit`；正式收尾使用 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` | 生成单一 `v1-completion-audit.json` / `.md`，汇总自动化命令状态、人工证据记录 completion 校验和 goal 是否可 complete；提供 `--manual-record` 时同目录归档 `manual-evidence-gaps.md` | 已有命令；默认不跑重型命令，只生成缺口审计；正式 completion audit 必须显式运行自动化命令并提供补证后的人工记录 |
...
5. `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` 生成的 `v1-completion-audit.json` / `.md` 结论为 `passed`，并归档自动化命令、人工证据记录路径和同目录 `manual-evidence-gaps.md`。
