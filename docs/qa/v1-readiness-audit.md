...
| iOS 系统能力人工证据记录 | `pnpm validate:ios-manual-evidence-record`；`pnpm prepare:ios-manual-review-pack -- --record <review-path>`；`pnpm prepare:ios-manual-evidence-record -- --record <review-path> --output <filled-path>`；最终验收使用 `node scripts/validate-ios-manual-evidence-record.mjs --record <path> --require-complete --report <report-path>` | 校验人工证据记录结构；从 review 记录生成 Markdown 复核包和 filled 草稿；completion 模式要求每项 `passed` 且证据字段齐全；`--report` 生成补证缺口报告 | 已有命令，当前只能证明模板结构、review pack 和 filled 草稿入口可校验；review pack 只是操作者复核工作台，不代表人工验收通过；真实 completion 仍需人工复核并签署补证文件 |
