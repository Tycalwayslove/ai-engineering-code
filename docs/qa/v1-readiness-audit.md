...
| v1 completion audit 汇总 | `pnpm collect:v1-completion-audit`；正式收尾使用 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>`，也可用 `--manual-record best --manual-record-root .tmp/ios-acceptance-evidence` 自动选择当前缺口最少的候选记录 | 生成单一 `v1-completion-audit.json` / `.md`，汇总自动化命令状态、人工证据记录 completion 校验、`manualEvidence.selection` 自动选取依据、`externalKnowledgeSync` 外部知识库同步状态和 goal 是否可 complete；提供 `--manual-record` 时同目录归档 `manual-evidence-gaps.md` | 已有命令；默认不跑重型命令，只生成缺口审计；正式 completion audit 必须显式运行自动化命令并提供补证后的人工记录；`best` 只负责选记录，不会把 review / draft 自动视为通过；外部知识库真实同步后可用 `--external-knowledge-status synced` 或 `--external-knowledge-synced` 记录证据 |
...
5. `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` 或 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record best --manual-record-root .tmp/ios-acceptance-evidence` 生成的 `v1-completion-audit.json` / `.md` 结论为 `passed`，并归档自动化命令、人工证据记录路径、`manualEvidence.selection` 选取依据、同目录 `manual-evidence-gaps.md` 和 `externalKnowledgeSync` 状态；若实际执行了飞书 / Obsidian 同步，需要通过 `--external-knowledge-status synced|partial`、`--feishu-sync-evidence <path>` 或 `--obsidian-sync-evidence <path>` 记录证据。
