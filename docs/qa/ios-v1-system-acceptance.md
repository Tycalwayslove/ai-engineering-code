...
如果要把 `manual-evidence-record.review.json` 整理成给操作者复核的 Markdown 清单，可以运行：
...
正式签署前建议先生成 `manual-evidence-review-pack.md`。该文件会汇总 record 路径、record HEAD、当前 HEAD、新鲜度、`acceptanceVerdict`、每个 item 的状态、缺少的候选证据、已预填的截图 / 录屏 / API 摘要 / bridge marker / 系统 artifact，以及下一步签署命令。它是复核工作台，不是通过证明；只要 review 记录仍是 `pending` / `not_evaluated`，completion audit 就必须保持 `not_complete`。
