...
`--seed-supported-system-evidence` / `AI_CODE_IOS_ACCEPTANCE_SEED_SUPPORTED_SYSTEM_EVIDENCE=1` 是上述自动证据包的批量 opt-in 开关：它同时启用验收事实种子、系统 Calendar App 截图、系统日历取消清理、日历权限拒绝降级、通知点击回流和通知 delivered 诊断，适合在一次采证中集中准备所有已支持的系统辅助材料。该开关可能创建 / 取消 seed 事项、撤销并恢复日历权限，以及等待短提醒到期；它只减少执行成本，不改变人工验收和 completion audit 的边界。
