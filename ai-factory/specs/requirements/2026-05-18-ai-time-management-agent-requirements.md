...
本文件是 2026-05-18 的第一版需求基线。2026-05-19 用户基于参考产品截图、视频笔记和 UI 评审反馈，确认产品方向需要修订。
...
- 产品从“日程、待办、提醒三类时间对象工作台”收敛为“AI 日程执行 Agent”。
- V1 先只做日程管理领域，待办、提醒、费用管理和外部连接器后置。
- 首页主路径从工作台/日历转为 AI 执行流。
- 后端负责指令解析、补全、规划、风险分级、确认和执行。
- Timeline Drawer 作为日期上下文抽屉，不再是普通侧边菜单。
