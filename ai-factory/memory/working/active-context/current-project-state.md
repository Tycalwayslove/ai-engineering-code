...
8. 正式 completion audit 时，用 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path>` 统一生成 `v1-completion-audit.json` / `.md` 和同目录 `manual-evidence-gaps.md`，把自动化门禁、人工证据记录和补证缺口报告绑定到单一收尾产物。
