...
- 2026-06-08 已提交 `ed0b281 feat(qa): 链接人工复核证据项`：`manual-evidence-review-pack.html` 的每个验收 item 现在带稳定锚点 `item-<itemId>`，`manual-acceptance-handoff.html` 的逐项 checkbox 清单增加“查看证据”链接，可直接跳到同目录 HTML Review Pack 的对应证据项；`validate:native-shells` 已守住 `itemAnchorId`、证据锚点和 Handoff 链接，避免后续回退。已运行 `node --test scripts/generate-ios-manual-review-pack.test.mjs scripts/prepare-ios-manual-acceptance-handoff.test.mjs scripts/validate-native-shells.test.mjs`、`node --check scripts/generate-ios-manual-review-pack.mjs && node --check scripts/prepare-ios-manual-acceptance-handoff.mjs && node --check scripts/validate-native-shells.mjs && git diff --check`、`pnpm validate:native-shells`，均通过。
- 2026-06-08 已用 HEAD `ed0b281` 生成 fresh evidence / Handoff / lightweight completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-ed0b281-lan`，HTML Handoff `.tmp/ios-acceptance-evidence/current-head-final-20260608-ed0b281-lan/manual-acceptance-handoff.html`，HTML Review Pack `.tmp/ios-acceptance-evidence/current-head-final-20260608-ed0b281-lan/manual-evidence-review-pack.html`，轻量 audit `.tmp/v1-completion-audit/current-full-final-20260608-ed0b281-lan`。本轮没有使用 `--run-automated-commands` 重跑 21 个重型自动化门禁，且本机 `Info.plist` 当前 `H5DevServerURL` 为 `http://127.0.0.1:3000/?native=ios`，因此 review 记录 `manualEvidence.missingEvidenceCount=11`，主要缺 `h5_address_override` 的“局域网地址 App 启动截图”和系统日历写入 / 取消清理候选材料；该 fresh audit 只证明新 Handoff 锚点能力已可生成，不覆盖 HEAD `029fdf5` 的局域网 full audit 候选证据齐备结论。当前最新代码仍不能 complete：14 个人工验收 item 全部 `pending`，`acceptanceVerdict=not_evaluated`，外部知识库 `not_synced`。
- 2026-06-08 已提交 `d9618d8 test(qa): 隔离采证单测URL环境`：`collect-ios-acceptance-evidence.test.mjs` 的 dry-run 子进程现在通过 `testEnv()` 清理外部 `H5_DEV_SERVER_URL`、`AI_CODE_H5_NATIVE_BASE_URL` 和 `AI_CODE_API_BASE_URL`，避免正式 LAN full audit 传入局域网环境后污染“默认 127.0.0.1”单测断言。已在 LAN 环境下运行 `pnpm validate:ios-acceptance-evidence`，29 个测试全部通过。
- 2026-06-08 已用 HEAD `d9618d8` 和局域网地址重跑当前正式证据包、HTML Handoff 与 full completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-d9618d8-lan` 绑定 `headSha=d9618d8`，HTML Handoff 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-d9618d8-lan/manual-acceptance-handoff.html`，HTML Review Pack 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-d9618d8-lan/manual-evidence-review-pack.html`；full audit `.tmp/v1-completion-audit/current-full-final-20260608-d9618d8-lan` 使用 `--run-automated-commands` 后 21 个自动化命令全部 `passed`，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current`、`recordHeadSha=d9618d8`、`currentHeadSha=d9618d8`，最终仍为 `verdict=not_complete`。剩余阻塞已经收敛为 14 个人工验收 item 全部 `pending`、`acceptanceVerdict=not_evaluated`，以及外部知识库 `not_synced`。不能把 goal 标记为 complete。
- 2026-06-08 已增强人工验收 Handoff 的防误签逻辑：当 review record 仍存在候选证据缺口时，Markdown / HTML Handoff 会进入补证模式，只展示 Review Pack 打开命令和重新生成 Handoff 命令，不再输出包含 `--mark-passed` 的签署命令；HTML 中会显示“签署命令生成器已停用”。当候选证据缺口为 0 时，正式包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-d9618d8-lan/manual-acceptance-handoff.html` 仍正常保留签署命令生成器。已运行 `node --test scripts/generate-ios-manual-review-pack.test.mjs scripts/prepare-ios-manual-acceptance-handoff.test.mjs scripts/validate-native-shells.test.mjs`、`node --check scripts/prepare-ios-manual-acceptance-handoff.mjs` 和 `pnpm validate:native-shells`，均通过。该改动只防止缺候选证据时误签，不替代真实人工复核。
- 2026-06-08 已用 HEAD `1bb9d83` 和局域网地址重跑当前正式证据包、HTML Handoff 与 full completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-1bb9d83-lan` 绑定 `headSha=1bb9d83`，HTML Handoff 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-1bb9d83-lan/manual-acceptance-handoff.html`，HTML Review Pack 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-1bb9d83-lan/manual-evidence-review-pack.html`；Handoff 显示“当前没有缺少的候选证据”，并保留签署命令生成器。full audit `.tmp/v1-completion-audit/current-full-final-20260608-1bb9d83-lan` 使用 `--run-automated-commands` 后 21 个自动化命令全部 `passed`，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current`、`recordHeadSha=1bb9d83`、`currentHeadSha=1bb9d83`，最终仍为 `verdict=not_complete`。剩余阻塞仍是 14 个人工验收 item 全部 `pending`、`acceptanceVerdict=not_evaluated`，以及外部知识库 `not_synced`。不能把 goal 标记为 complete。
- 2026-06-08 已为正式 iOS 采证新增 LAN H5 前置校验：`collect:ios-acceptance-evidence` 支持 `--require-lan-h5` 和 `AI_CODE_IOS_ACCEPTANCE_REQUIRE_LAN_H5=1`，会把 `serviceHealth.h5NativeUrlKind`、`ios.h5DevServerUrlKind`、`ios.formalReadiness` 和 `manifest.formalReadiness` 写入证据包；当要求 LAN 但 `H5DevServerURL` 仍是 `localhost` / `127.*` / 非私有局域网地址时，脚本会在基础服务和 iOS 启动信息采集后立即写出 `acceptance-evidence.json` / `manifest.json` 并非零退出，不再继续跑系统日历、通知、附件等长采证。dry-run 已覆盖默认 loopback 失败和 `192.168.1.238` LAN override 成功；`validate:native-shells` 已守住参数、readiness 字段和失败文案。已运行 `node --test scripts/collect-ios-acceptance-evidence.test.mjs`、`node --check scripts/collect-ios-acceptance-evidence.mjs`、`node --test scripts/validate-native-shells.test.mjs`、`pnpm validate:ios-acceptance-evidence` 和 `pnpm validate:native-shells`，均通过。下一次正式 LAN full audit 应显式加 `--require-lan-h5`，防止 loopback 证据包被误当成正式验收候选。
- 2026-06-08 已用当前 HEAD `9e3c418` 和 `--require-lan-h5` 重跑正式 LAN 证据包、Handoff 与 full completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-9e3c418-lan` 绑定 `headSha=9e3c418`，`serviceHealth.h5NativeUrlKind=private_lan`、`ios.h5DevServerUrlKind=private_lan`、`ios.formalReadiness.ready=true`、`manifest.formalReadiness.ready=true`；HTML Handoff 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-9e3c418-lan/manual-acceptance-handoff.html`，HTML Review Pack 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-9e3c418-lan/manual-evidence-review-pack.html`。full audit `.tmp/v1-completion-audit/current-full-final-20260608-9e3c418-lan` 使用 `--run-automated-commands` 后 21 个自动化命令全部 `passed`，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current`、`recordHeadSha=9e3c418`、`currentHeadSha=9e3c418`，最终仍为 `verdict=not_complete`。剩余阻塞仍是 14 个人工验收 item 全部 `pending`、`acceptanceVerdict=not_evaluated`，以及外部知识库 `not_synced`。不能把 goal 标记为 complete。
- 2026-06-08 已修复 completion audit 的 `--manual-record best` 选择逻辑：候选排序现在把 `current_with_docs_only_changes` 纳入 freshness，优先级位于 exact `current` 之后、`missing_record_head/unknown` 和 `stale` 之前，并把被选中候选的 `selectedPostRecordChangedFiles` 传入最终 `packageFreshness`，避免文档提交后默认 Handoff / audit 误选旧 HEAD 的 `manual-evidence-record.filled.json`。已用 TDD 覆盖“旧 filled 包 vs docs-only 新鲜 review 包”和 `latest` mtime 选择，并用真实 `.tmp/ios-acceptance-evidence` 轻量 audit 验证 `best` 会选中 `.tmp/ios-acceptance-evidence/current-head-final-20260608-a00ae87-lan/manual-evidence-record.review.json`。
- 2026-06-08 已用当前 HEAD `a00ae87` 和 `--require-lan-h5` 重跑正式 LAN 证据包、Handoff 与 full completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-a00ae87-lan` 绑定 `headSha=a00ae87`，HTML Handoff 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-a00ae87-lan/manual-acceptance-handoff.html`，HTML Review Pack 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-a00ae87-lan/manual-evidence-review-pack.html`。full audit `.tmp/v1-completion-audit/current-full-final-20260608-a00ae87-lan` 使用 `--run-automated-commands --manual-record best --manual-record-root .tmp/ios-acceptance-evidence` 后 21 个自动化命令全部 `passed`，`manualEvidence.selection.selectedRecordPath=.tmp/ios-acceptance-evidence/current-head-final-20260608-a00ae87-lan/manual-evidence-record.review.json`，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current`、`recordHeadSha=a00ae87`、`currentHeadSha=a00ae87`，最终仍为 `verdict=not_complete`。剩余阻塞仍是 14 个人工验收 item 全部 `pending`、`acceptanceVerdict=not_evaluated`，以及外部知识库 `not_synced`。不能把 goal 标记为 complete。
- 2026-06-08 已加固人工验收 completion 签署闸门：`validate-ios-manual-evidence-record --require-complete` 现在要求 passed 记录包含 `operatorSignoff.mode=mark-passed`、非空 `operator`、ISO `confirmedAt`、完整且无重复的 `reviewedItemIds`，并在记录含 `headSha` 时要求 `operatorSignoff.recordHeadSha` 与之相同。`prepare:ios-manual-evidence-record -- --mark-passed` 会写入 `recordHeadSha`；默认 draft 和通知 metadata 导入路径仍保持 `pending` / `not_evaluated`。同时 `--manual-record best` 在同 freshness 候选里会优先选择 `postRecordChangedFiles` 更少、离当前 HEAD 更近的记录，避免所有候选都 stale 后旧 filled 包压过较新的 review 包；dry-run 占位 `headSha` 会标记为 `invalid_record_head` 并排在真实 stale 记录之后。已运行 `node --test scripts/validate-ios-manual-evidence-record.test.mjs scripts/fill-ios-manual-evidence-record.test.mjs scripts/collect-v1-completion-audit.test.mjs`、`node --test scripts/collect-v1-completion-audit.test.mjs`、`pnpm validate:ios-manual-evidence-record`、`pnpm validate:native-shells` 和轻量 audit `.tmp/v1-completion-audit/signoff-gate-check-20260608`。篡改 review JSON 为全 passed 但没有签署元数据时，`--require-complete` 会失败；当前 `a00ae87` review 记录仍因缺少 `operatorSignoff` 和 14 个 item pending 保持 `verdict=not_complete`。本改动触及验收脚本，提交后当前 `a00ae87` 正式证据包不再是 exact current；正式收尾仍需重新生成当前 HEAD 证据包或提供当前 HEAD 的 signed filled 记录。
- 2026-06-08 已用当前 HEAD `4281375` 和 `--require-lan-h5` 重跑正式 LAN 辅助证据包、Handoff 与 lightweight completion audit：证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan` 绑定 `headSha=4281375`，`manifest.formalReadiness.ready=true`、`serviceHealth.h5NativeUrlKind=private_lan`、`ios.h5DevServerUrlKind=private_lan`，review 记录 14 个 item 全部 `pending` 且 `missingEvidenceCount=0`。HTML Handoff 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan/manual-acceptance-handoff.html`，HTML Review Pack 路径为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan/manual-evidence-review-pack.html`，确认列表为 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan/manual-evidence-reviewed-items.json`。lightweight audit `.tmp/v1-completion-audit/current-lightweight-20260608-4281375-lan` 显示 `manualEvidence.packageFreshness.status=current`、`recordHeadSha=4281375`、`currentHeadSha=4281375`、`manualEvidence.missingEvidenceCount=0`，最终仍为 `verdict=not_complete`。本轮未使用 `--run-automated-commands` 重跑 21 个自动化命令；`a00ae87` full audit 仍是最近一次全量自动化基线，但正式 completion 仍需当前 HEAD 的 signed filled 记录并重新跑 full audit。
- 2026-06-08 已用当前 HEAD `a4b9385` 补跑 full completion audit：`.tmp/v1-completion-audit/current-full-final-20260608-a4b9385-lan` 使用 `--run-automated-commands --manual-record best --manual-record-root .tmp/ios-acceptance-evidence` 后 21 个自动化命令全部 `passed`，自动选择 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan/manual-evidence-record.review.json`，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current_with_docs_only_changes`、`recordHeadSha=4281375`、`currentHeadSha=a4b9385`。后续变更仅包含 `ai-factory/memory/` 与 `docs/` 源稿，未发现产品代码旧 HEAD 阻塞。最终仍为 `verdict=not_complete`，原因是 14 个人工验收 item 全部 `pending`、缺少 `operatorSignoff`，且外部知识库 `not_synced`。不能把 goal 标记为 complete。
- 2026-06-10 已为 iOS 人工验收 Handoff / Review Pack 新增机器预检摘要：生成器会统计每个 item 的必需候选证据是否齐备，输出 `machinePrecheck: candidateEvidenceComplete=<n>/<total>, incomplete=<m>, requiresHumanSignoff=true`，并在逐项证据段显示“候选证据齐备；仍需人工复核签署”或缺口数量。该预检只降低人工复核定位成本，不改变 `pending` / `not_evaluated`，也不能替代 `operatorSignoff`。已基于 `.tmp/ios-acceptance-evidence/current-head-final-20260608-4281375-lan/manual-evidence-record.review.json` 重新生成 Handoff / Review Pack，当前产物显示 `machinePrecheck=14/14`，但正式 completion 仍需要真实操作者逐项复核并签署 passed filled record 后重跑 full audit。
- 2026-06-10 已用当前 HEAD `94a9a75` 和本项目 H5 `http://192.168.1.238:3100/?native=ios&bridgeDebug=1` 重跑正式 LAN 辅助证据包、Handoff 与 lightweight completion audit：采证前发现 3000 端口被另一个 `meu-mall` Next dev server 占用，因此改用 3100 启动本项目 H5；错误采证半包已删除。新证据包 `.tmp/ios-acceptance-evidence/current-head-final-20260610-94a9a75-lan` 绑定 `manifest.headSha=94a9a75`、`manual-evidence-record.review.json.headSha=94a9a75`，`manifest.formalReadiness.ready=true`、`serviceHealth.h5NativeUrlKind=private_lan`、`ios.h5DevServerUrlKind=private_lan`，`acceptanceFactSeed`、`calendarCleanupSeed`、`notificationUiTest`、`nativeKeyboardInput` 和 `nativeAttachmentInputs` 均可用。HTML Handoff / HTML Review Pack 已生成，同目录机器预检为 `candidateEvidenceComplete=14/14, incomplete=0, requiresHumanSignoff=true`。lightweight audit `.tmp/v1-completion-audit/current-lightweight-20260610-94a9a75-lan` 自动选择当前 `94a9a75` review record，`manualEvidence.missingEvidenceCount=0`、`manualEvidence.packageFreshness.status=current`、`recordHeadSha=94a9a75`、`currentHeadSha=94a9a75`，最终仍为 `verdict=not_complete`，因为 14 个 item 全部 `pending`、缺少 `operatorSignoff`，且外部知识库 `not_synced`。本轮未重跑 `--run-automated-commands` full audit；正式 completion 仍需 signed filled record 后重跑 full audit。
