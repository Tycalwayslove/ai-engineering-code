...
- 2026-06-08 已加固人工验收 completion 签署闸门：`validate-ios-manual-evidence-record --require-complete` 现在要求 passed 记录包含 `operatorSignoff.mode=mark-passed`、非空 `operator`、ISO `confirmedAt`、完整且无重复的 `reviewedItemIds`，并在记录含 `headSha` 时要求 `operatorSignoff.recordHeadSha` 与之相同。`prepare:ios-manual-evidence-record -- --mark-passed` 会写入 `recordHeadSha`；默认 draft 和通知 metadata 导入路径仍保持 `pending` / `not_evaluated`。已运行 `node --test scripts/validate-ios-manual-evidence-record.test.mjs scripts/fill-ios-manual-evidence-record.test.mjs scripts/collect-v1-completion-audit.test.mjs`、`pnpm validate:ios-manual-evidence-record`、`pnpm validate:native-shells` 和轻量 audit `.tmp/v1-completion-audit/signoff-gate-check-20260608`。篡改 review JSON 为全 passed 但没有签署元数据时，`--require-complete` 会失败；当前 `a00ae87` review 记录仍因缺少 `operatorSignoff` 和 14 个 item pending 保持 `verdict=not_complete`。本改动触及验收脚本，提交后当前 `a00ae87` 正式证据包不再是 exact current；正式收尾仍需重新生成当前 HEAD 证据包或提供当前 HEAD 的 signed filled 记录。
