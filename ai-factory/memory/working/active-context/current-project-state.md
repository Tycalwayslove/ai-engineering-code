...
8. 正式 completion audit 时，用 `pnpm collect:v1-completion-audit -- --run-automated-commands --manual-record <path> --external-knowledge-status not_synced` 统一生成 `v1-completion-audit.json` / `.md` 和同目录 `manual-evidence-gaps.md`，把自动化门禁、人工证据记录、补证缺口报告和外部知识库同步状态绑定到单一收尾产物；若已真实同步飞书或 Obsidian，改用 `synced|partial` 并补对应 evidence 参数。
