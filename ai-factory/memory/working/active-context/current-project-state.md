...
- 2026-05-29 已新增 iOS 原生键盘 UI test 门禁：Xcode 工程新增 `AIEngineeringCodeUITests` target 和共享 scheme，根命令 `pnpm validate:ios-keyboard-ui-test` 会运行 `NativeKeyboardInputUITests.testNativeKeyboardComposerSubmitsThroughH5Bridge`，真实点击 Native composer mode toggle、TextField、等待系统键盘、输入“明天上午十点提醒我带电脑”、点击发送，并在 H5 Bridge Debug 中断言 `source=native.composer.keyboard` 和提交文本。UI test 通过专用启动参数让通知 / 日历同步只回 ack、不请求系统权限，避免系统权限弹窗污染键盘路径；该命令已在本机 Simulator 通过 1 个测试、0 个失败。它降低键盘路径回归风险，但仍不替代最终人工验收记录里的截图、后端接口摘要和验收结论。
