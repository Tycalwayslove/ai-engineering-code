...
const destination =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_DESTINATION ??
...
const uiTestTarget =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_TARGET ??
  "AIEngineeringCodeUITests";
const uiTestClass =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_CLASS ??
...
const artifactDir =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_ARTIFACT_DIR ??
  path.join(".tmp", "ios-keyboard-ui-test");
const logPath =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_LOG_PATH ??
  path.join(artifactDir, "ios-keyboard-ui-test.log");
const metadataPath =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_METADATA_PATH ??
  path.join(artifactDir, "keyboard-ui-test.json");
const resultBundlePath =
  process.env.AI_CODE_IOS_KEYBOARD_UI_TEST_RESULT_BUNDLE_PATH ??
  path.join(artifactDir, "ios-keyboard-ui-test.xcresult");
...
if (version.error) {
  console.error("iOS keyboard UI test failed: xcodebuild is not available.");
  console.error(
...
if (version.status !== 0) {
  console.error("iOS keyboard UI test failed while checking xcodebuild.");
    ...
    `${JSON.stringify(
      {
        ...
        sourceMarkers: ["source=native.composer.keyboard"],
      },
      null,
      2
    )}\n`
...
if (result.error?.code === "ETIMEDOUT") {
  ...
  console.error(
    `iOS keyboard UI test timed out after ${timeoutMs}ms. Set AI_CODE_IOS_KEYBOARD_UI_TEST_TIMEOUT_MS to override.`
  );
  process.exit(1);
}
...
if (result.error) {
  ...
  console.error(`iOS keyboard UI test failed: ${result.error.message}`);
  process.exit(1);
}
