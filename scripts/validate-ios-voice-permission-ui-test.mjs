...
const destination =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_DESTINATION ??
...
const uiTestClass =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_CLASS ??
  "NativeKeyboardInputUITests";
const uiTestMethod =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_METHOD ??
  "testNativeVoicePermissionPromptCanBeCaptured";
const timeoutMs = Number.parseInt(
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_TIMEOUT_MS ?? "900000",
  10
);
const artifactDir =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_ARTIFACT_DIR ??
  path.join(".tmp", "ios-voice-permission-ui-test");
const logPath =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_LOG_PATH ??
  path.join(artifactDir, "ios-voice-permission-ui-test.log");
const metadataPath =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_METADATA_PATH ??
  path.join(artifactDir, "voice-permission-ui-test.json");
const resultBundlePath =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_RESULT_BUNDLE_PATH ??
  path.join(artifactDir, "ios-voice-permission-ui-test.xcresult");
const shouldResetPrivacy =
  process.env.AI_CODE_IOS_VOICE_PERMISSION_RESET_PRIVACY !== "0";
...
if (version.error) {
  console.error("iOS voice permission UI test failed: xcodebuild is not available.");
  console.error(
...
if (version.status !== 0) {
  console.error("iOS voice permission UI test failed while checking xcodebuild.");
...
console.log(
  [
    "Running iOS voice permission UI test:",
    "xcodebuild",
...
if (result.error?.code === "ETIMEDOUT") {
  ...
  console.error(
    `iOS voice permission UI test timed out after ${timeoutMs}ms. Set AI_CODE_IOS_VOICE_PERMISSION_UI_TEST_TIMEOUT_MS to override.`
  );
  process.exit(1);
}
...
if (result.error) {
  ...
  console.error(`iOS voice permission UI test failed: ${result.error.message}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  ...
  console.error(
    `iOS voice permission UI test failed with exit code ${result.status ?? "unknown"}.`
  );
...
if (!/Executed\s+[1-9]\d*\s+tests?[, ]/.test(combinedOutput)) {
  ...
  console.error(
    "iOS voice permission UI test failed: xcodebuild completed without executing the selected XCTest."
  );
  process.exit(1);
}

writeMetadata({ passed: true });
console.log("iOS voice permission UI test passed.");
